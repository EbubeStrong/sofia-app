...
import { AdmissionLayoutStats } from "@/data/pharmacy-data";
...
import AdmissionQueueLayoutTable from "./AdmissionQueueLayoutTable";
...
const AdmissionLayoutModule: React.FC<TQueueProps> = ({ initialData, hospitals }) => {
  ...
  // const resolvedStats = AdmissionLayoutStats.map((stat) => {
  //   let amount = stat.amount;

  //   if (statsFromApi) {
  //     if (stat.id === "pendingRequests") {
  //       amount = String(statsFromApi.totalPendingRequests ?? 0);
  //     }
  ...
  return (
    <div className="flex flex-col gap-7">
      <section className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div className="flex-1">
          ...
          <p className="text-base text-[#101010]/70 font-normal font-libre_franklin">
            Patients admission and bed management system
          </p>
        </div>
        <Space size="small">
          <Button
          ...
          >
            <FaPlus />  Discharge Request
          </Button>
        </Space>
      </section>

      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {AdmissionLayoutStats.map((stat) => (
          <DCard $type={stat.id} key={stat.id} className="relative">
            <DTitle $type={stat.id}>{stat.title}</DTitle>

            <div className="flex flex-col gap-2 justify-center">
              <p
                className={`${stat.id === "pendingRequests" ? "text-white" : "text-sofia_dark"
      ...
      <AdmissionQueueLayoutTable tableData={initialData} type="queue" hospitals={hospitals} />
...
export default AdmissionLayoutModule;