...
import { NurseDashboardStats } from "@/data/nurses-note-data";
import NursingQueueTable from "../nurses-note/NursingQueueTable";
import { NursesStatsResp, TNursingQueueResp } from "@/interfaces/nurses";
import { useFetchNursingQueue, useNursesStats } from "@/services/nurses";
...
const NursesDashboardLayout: React.FC<TQueueProps> = ({
  initialData,
  isLead,
  tabQuery,
}) => {
  ...
  const [getStats, setGetStats] = useState({} as NursesStatsResp["data"]);

  const { data: nurseStatsData, isSuccess: isNurseStatSuccess } =
    useNursesStats();

  const { data: nursingQueue, isFetching: isFetchingQueue } =
    useFetchNursingQueue(initialData, activeKey, tabQuery);

  useEffect(() => {
    if (isNurseStatSuccess && nurseStatsData) {
      setGetStats(nurseStatsData);
    }
  }, [nurseStatsData, isNurseStatSuccess]);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {NurseDashboardStats(getStats, isLead)?.map((stat) => (
          <StatsCardVariant
            key={stat.id}
            id={stat.id}
            title={stat.title}
      ...
      {/* Temporarily hidden until the APIs are available */}
      <section className="hidden flex-col xl:flex-row gap-5 w-full max-w-full">
...
export default NursesDashboardLayout;
