...
import { usePharmacyProfile, useUpdatePharmacyProfile } from "@/services/pharmacy";
...
const PharmacyBasicDetails: React.FC<PharmacyDetailsProps> = ({ role }) => {
  ...
  const { mutate, isPending } = useUpdatePharmacyProfile();

  const details = [
    {
      title: "User ID",
      desc: profile?.pharmacistId ?? "---",
    },
    {
      title: "Date Joined",
    ...
    mutate(
      { body: payload },
      {
        onSuccess: () => {
          ...
          invalidateQuery(["pharmacy_profile"]);
          form.resetFields();
        },
      }
    );
