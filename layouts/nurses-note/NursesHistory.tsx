...
import { usePatientNursesNotes } from "@/services/patients";
...
interface NursesHistoryProps {
  patientId: string;
}

const NursesHistoryLayout: React.FC<NursesHistoryProps> = ({ patientId }) => {
  ...
  const { data: patientNursesNotes, isFetching: isFetchingNotes } =
    usePatientNursesNotes(patientId, pageNumber, pageSize);
  ...
  if (isFetchingNotes) {
    return (
      <ComponentLoader label="Loading recent nurse's note..." height={380} />
    );
  }

  return (
    <div>
      {(patientNursesNotes?.data?.length ?? 0) > 0 ? (
        <div className="flex flex-col gap-5">
          <HistoryHeader
            title="Nurse's Note History"
            desc="Chronological history of previous notes"
          />

          <div className="flex flex-col gap-4">
            {patientNursesNotes?.data?.map((item) => (
              <div
                key={item.id}
          ...
          {(patientNursesNotes?.data?.length ?? 0) > 10 && (
            <SofiaPagination
              onChange={handlePageChange}
              align="end"
              current={patientNursesNotes?.page ?? 1}
              pageSize={patientNursesNotes?.perPage ?? 10}
              total={patientNursesNotes?.totalCount}
            />
          )}
        </div>
      ) : (
        <div className="flex justify-center">
          <EmptyTable
            message="No recent nurse's note found"
...
export default NursesHistoryLayout;
